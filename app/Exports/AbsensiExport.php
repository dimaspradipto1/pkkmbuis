...
class AbsensiExport implements FromCollection, WithHeadings, WithEvents, ShouldAutoSize
{
    protected string $modelClass;
    protected string $title;

    public function __construct(string $modelClass, string $title = 'DATA ABSENSI MAHASISWA - PKKMB UIS 2026')
    {
    ...
    public function collection()
    {
        ...
        return $items->map(function ($item, $index) {
            $catatanDatang = $item->catatan_datang;
            if (empty($catatanDatang) && !empty($item->catatan)) {
                if (str_contains($item->catatan, 'Datang:')) {
                    preg_match('/Datang:\s*([^|]+)/', $item->catatan, $m);
                    $catatanDatang = trim($m[1] ?? '');
                } else {
                    $catatanDatang = $item->catatan;
                }
            }

            $catatanPulang = $item->catatan_pulang;
            if (empty($catatanPulang) && !empty($item->catatan) && str_contains($item->catatan, 'Pulang:')) {
                preg_match('/Pulang:\s*([^|]+)/', $item->catatan, $m);
                $catatanPulang = trim($m[1] ?? '');
            }
            ...
            return [
                ...
                'hadir_pagi'     => $item->hadir_pagi ?? '-',
                'waktu_datang'   => $item->waktu_datang ? \Carbon\Carbon::parse($item->waktu_datang)->format('H:i:s') : '-',
                'catatan_datang' => !empty($catatanDatang) ? $catatanDatang : '-',
                'hadir_sore'     => $item->hadir_sore ?? '-',
                'waktu_pulang'   => $item->waktu_pulang ? \Carbon\Carbon::parse($item->waktu_pulang)->format('H:i:s') : '-',
                'catatan_pulang' => !empty($catatanPulang) ? $catatanPulang : '-',
            ];
        });
    }
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                if ($highestRow >= 5) {
                    ...
                    // Align center status/time columns
