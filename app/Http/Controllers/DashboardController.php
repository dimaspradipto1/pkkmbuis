...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($isPendamping) {
            ...
            $data = Cache::remember($cacheKey, 60, function () use ($authUser) {
                ...
                $disiplinUserIds = collect()
                    ->merge(KedisiplinanPertama::whereIn('user_id', $targetUserIds)->pluck('user_id'))
                    ->merge(KedisiplinanKedua::whereIn('user_id', $targetUserIds)->pluck('user_id'))
                    ->merge(KedisiplinanKetiga::whereIn('user_id', $targetUserIds)->pluck('user_id'))
                    ->unique();
        ...
        $globalData = Cache::remember('dashboard_global_stats', 60, function () {
            ...
            $disiplinUserIds = collect()
                ->merge(KedisiplinanPertama::pluck('user_id'))
                ->merge(KedisiplinanKedua::pluck('user_id'))
                ->merge(KedisiplinanKetiga::pluck('user_id'))
                ->unique();
            $disiplinCount = $disiplinUserIds->count();
