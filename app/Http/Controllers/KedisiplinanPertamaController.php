...
class KedisiplinanPertamaController extends Controller
{
    ...
    public function index(KedisiplinanPertamaDataTable $dataTable)
    {
        ...
        if ($authUser->role == 'kakakpendamping') {
            ...
            $sudahAdaData = KedisiplinanPertama::whereIn('user_id', $targetUserIds)
                ->where(function($q) {
            ...
            $totalMahasiswa = 1;
            $hasData = KedisiplinanPertama::where('user_id', $authUser->id)
                ->where(function($q) {
                    $q->where(function($sub) {
                        $sub->whereNotNull('kelengkapan_atribut')->where('kelengkapan_atribut', '!=', '')->where('kelengkapan_atribut', '!=', '-');
                    })->orWhere(function($sub) {
            ...
            $sudahAdaData = KedisiplinanPertama::whereHas('user', fn($q) => $q->where('role', 'mahasiswa'))
                ->where(function($q) {
                    $q->where(function($sub) {
                        $sub->whereNotNull('kelengkapan_atribut')->where('kelengkapan_atribut', '!=', '')->where('kelengkapan_atribut', '!=', '-');
                    })->orWhere(function($sub) {
        ...
        $notes = \App\Models\KedisiplinanNote::where('category', 'kedisiplinanpertama')->latest()->get();

        return $dataTable->render('pages.kedisiplinanpertama.index', compact('attachments', 'notes', 'totalMahasiswa', 'sudahAdaData', 'belumAdaData'));
    }
    ...
    public function create()
    {
        ...
        if ($authUser->role == 'kakakpendamping') {
            ...
            $users = User::where('role', 'mahasiswa')->whereIn('kelompok_id', $myKelompokIds)->with(['kelompok', 'kedisiplinanPertama'])->orderBy('name')->get();
        } elseif ($authUser->role == 'dosenpendamping') {
            $myKelompokIds = \App\Models\Kelompok::whereHas('dosenPendampings', fn($q) => $q->where('users.id', $authUser->id))->pluck('id');
            $users = User::where('role', 'mahasiswa')->whereIn('kelompok_id', $myKelompokIds)->with(['kelompok', 'kedisiplinanPertama'])->orderBy('name')->get();
        } else {
            $users = User::where('role', 'mahasiswa')->with(['kelompok', 'kedisiplinanPertama'])->orderBy('name')->get();
        }
    ...
    public function store(Request $request)
    {
        ...
        KedisiplinanPertama::updateOrCreate(
    ...
    public function edit(string $id)
    {
        ...
        if ($authUser->role == 'kakakpendamping') {
            $myKelompokIds = \App\Models\Kelompok::where('pendamping_id', $authUser->id)->orWhereHas('kakakPendampings', fn($q) => $q->where('users.id', $authUser->id))->pluck('id');
            $users = User::where('role', 'mahasiswa')->where(fn($q) => $q->whereIn('kelompok_id', $myKelompokIds)->orWhere('id', $kedisiplinanPertama->user_id))->with(['kelompok', 'kedisiplinanPertama'])->orderBy('name')->get();
        } elseif ($authUser->role == 'dosenpendamping') {
            $myKelompokIds = \App\Models\Kelompok::whereHas('dosenPendampings', fn($q) => $q->where('users.id', $authUser->id))->pluck('id');
            $users = User::where('role', 'mahasiswa')->where(fn($q) => $q->whereIn('kelompok_id', $myKelompokIds)->orWhere('id', $kedisiplinanPertama->user_id))->with(['kelompok', 'kedisiplinanPertama'])->orderBy('name')->get();
        } else {
    ...
    public function update(Request $request, string $id)
    {
        ...
        $existingOther = KedisiplinanPertama::where('user_id', $validated['user_id'])
            ->where('id', '!=', $id)
            ->first();
    ...
    public function bulkDelete(Request $request)
    {
        ...
        KedisiplinanPertama::whereIn('id', $request->ids)->delete();
