...
class EvaluasiFstDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<EvaluasiFst> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                $showBtn = '<a href="' . route('evaluasifst.show', $item->id) . '" class="btn btn-sm btn-info text-white me-1" title="Detail"><i class="bi bi-eye"></i></a>';
                $editBtn = '<a href="' . route('evaluasifst.edit', $item->id) . '" class="btn btn-sm btn-primary me-1" title="Edit"><i class="bi bi-pencil"></i></a>';
                $deleteBtn = '';
                if (Auth::user()->role == 'admin') {
                    $deleteBtn = '<form action="' . route('evaluasifst.destroy', $item->id) . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus evaluasi ini?\')">'
     ...
     *
     * @return QueryBuilder<EvaluasiFst>
     */
    public function query(EvaluasiFst $model): QueryBuilder
    {
    ...
    {
        return 'EvaluasiFst_' . date('YmdHis');
    }
}
