...
class EvaluasiFebDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<EvaluasiFeb> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                $showBtn = '<a href="' . route('evaluasifeb.show', $item->id) . '" class="btn btn-sm btn-info text-white me-1" title="Detail"><i class="bi bi-eye"></i></a>';
                $editBtn = '<a href="' . route('evaluasifeb.edit', $item->id) . '" class="btn btn-sm btn-primary me-1" title="Edit"><i class="bi bi-pencil"></i></a>';
                $deleteBtn = '';
                if (Auth::user()->role == 'admin') {
                    $deleteBtn = '<form action="' . route('evaluasifeb.destroy', $item->id) . '" method="POST" class="d-inline" onsubmit="return confirm(\'Apakah Anda yakin ingin menghapus evaluasi ini?\')">'
     ...
     *
     * @return QueryBuilder<EvaluasiFeb>
     */
    public function query(EvaluasiFeb $model): QueryBuilder
    {
    ...
    {
        return 'EvaluasiFeb_' . date('YmdHis');
    }
}
