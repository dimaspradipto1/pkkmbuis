...
class ObservasiAcaraFstDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<ObservasiAcaraFst> $query Results from query() method.
     */
            ...
            ->addColumn('action', function ($item) {
                return '
                    <div class="d-flex justify-content-center gap-1">
                        <a href="' . route('observasiacarafst.edit', $item->id) . '" class="btn btn-sm btn-warning text-white px-3 rounded"><i class="fa-solid fa-pen-to-square"></i></a>
                        <form action="' . route('observasiacarafst.destroy', $item->id) . '" method="POST" style="display: inline">
                            ' . csrf_field() . '
                            ' . method_field('DELETE') . '
     ...
     *
     * @return QueryBuilder<ObservasiAcaraFst>
     */
    public function query(ObservasiAcaraFst $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
                    ->setTableId('observasiacarafst-table')
    ...
    {
        return 'ObservasiAcaraFst_' . date('YmdHis');
    }
}
