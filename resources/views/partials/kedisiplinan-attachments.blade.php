...
<div class="card shadow-sm border-0 mt-4 rounded-3">
    ...
    <div class="card-body">
        ...
        @if (Auth::check() && Auth::user()->role != 'mahasiswa')
            <!-- Upload Form -->
            <form action="{{ route('kedisiplinan-attachments.store') }}" method="POST" enctype="multipart/form-data" class="bg-light p-3 rounded-3 border mb-4">
                @csrf
                    ...
                    <div class="col">
                        <div class="card h-100 border shadow-sm rounded-3 overflow-hidden">
                            ...
                            <div class="bg-light text-center p-3 border-bottom position-relative d-flex align-items-center justify-content-center" style="height: 180px;">
                                ...
                                @if (Auth::check() && Auth::user()->role != 'mahasiswa')
                                    <form action="{{ route('kedisiplinan-attachments.destroy', $item->id) }}" method="POST"
                            ...
                            <div class="card-body p-3 d-flex flex-column justify-content-between">
                                ...
                                <div class="pt-2 border-top">
                                    ...
                                    <div class="input-group input-group-sm mb-2">
                                        <input type="text" readonly value="{{ $fileUrl }}" id="kedisiplinan_file_link_{{ $item->id }}" class="form-control form-control-sm bg-light text-dark extra-small" style="font-size: 0.75rem;">
                                        <button class="btn btn-outline-secondary extra-small fw-bold px-3" type="button" onclick="copyAttachmentLink('kedisiplinan_file_link_{{ $item->id }}')" title="Salin Link">
