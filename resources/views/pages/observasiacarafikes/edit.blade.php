...
@section('content')
    <div class="pagetitle">
        <h1>Edit Observasi Acara Fikes</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('observasiacarafikes.index') }}">Observasi Acara Fikes</a></li>
    ...
    <section class="section">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Form Edit Observasi Acara Fikes</h5>

                        <form action="{{ route('observasiacarafikes.update', $observasiAcaraFikes->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="waktu_runddown" id="waktu_runddown" class="form-control" value="{{ old('waktu_runddown', $observasiAcaraFikes->waktu_runddown) }}">
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="waktu_realisasi" id="waktu_realisasi" class="form-control" value="{{ old('waktu_realisasi', $observasiAcaraFikes->waktu_realisasi) }}">
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <textarea name="kegiatan" id="kegiatan" class="form-control" rows="3" required>{{ old('kegiatan', $observasiAcaraFikes->kegiatan) }}</textarea>
                                </div>
                            </div>

                            @php
                                $aspekItems = old('aspek_observasi');
                                if (!$aspekItems) {
                                    $aspekItems = collect(explode("\n", (string) $observasiAcaraFikes->aspek_observasi))
                                ...
                                <div class="col-sm-10 d-flex align-items-center">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <div class="form-check form-check-inline me-4">
                                            <input class="form-check-input" type="radio" name="skala" id="skala_{{ $i }}" value="{{ $i }}" {{ old('skala', $observasiAcaraFikes->skala) == $i ? 'checked' : '' }}>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <textarea name="catatan" id="catatan" class="form-control" rows="3">{{ old('catatan', $observasiAcaraFikes->catatan) }}</textarea>
                                </div>
                            </div>

                            @php
                                $linkItems = old('link_dokumen');
                                if (!$linkItems) {
                                    $linkItems = collect(explode("\n", (string) $observasiAcaraFikes->link_dokumen))
                            ...
                            <div class="text-end mt-4">
                                <button type="submit" class="btn btn-warning text-white">Update Observasi</button>
                                <a href="{{ route('observasiacarafikes.index') }}" class="btn btn-secondary">Batal</a>
                            </div>
                        </form>
