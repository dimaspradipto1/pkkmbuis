            ...
            <div class="col-lg-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('absenpertama.update', $absenPertama->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9 d-flex align-items-center">
                                    <div class="form-check form-check-inline me-4">
                                        <input class="form-check-input" type="radio" name="hadir_pagi" id="pagi_hadir" value="Hadir" {{ old('hadir_pagi', $absenPertama->hadir_pagi) == 'Hadir' ? 'checked' : '' }}>
                                        <label class="form-check-label fw-semibold" for="pagi_hadir">Hadir</label>
                                    </div>
                                    <div class="form-check form-check-inline me-4">
                                        <input class="form-check-input" type="radio" name="hadir_pagi" id="pagi_izin" value="Izin" {{ old('hadir_pagi', $absenPertama->hadir_pagi) == 'Izin' ? 'checked' : '' }}>
                                        <label class="form-check-label fw-semibold" for="pagi_izin">Izin</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="hadir_pagi" id="pagi_tidak" value="Tidak Hadir" {{ old('hadir_pagi', $absenPertama->hadir_pagi) == 'Tidak Hadir' ? 'checked' : '' }}>
                            ...
                            <div class="row mb-3" id="container_hadir_sore">
                                ...
                                <div class="col-sm-9 d-flex align-items-center">
                                    <div class="form-check form-check-inline me-4">
                                        <input class="form-check-input" type="radio" name="hadir_sore" id="sore_hadir" value="Hadir" {{ old('hadir_sore', $absenPertama->hadir_sore) == 'Hadir' ? 'checked' : '' }}>
                                        <label class="form-check-label fw-semibold" for="sore_hadir">Hadir</label>
                                    </div>
                                    <div class="form-check form-check-inline me-4">
                                        <input class="form-check-input" type="radio" name="hadir_sore" id="sore_izin" value="Izin" {{ old('hadir_sore', $absenPertama->hadir_sore) == 'Izin' ? 'checked' : '' }}>
                                        <label class="form-check-label fw-semibold" for="sore_izin">Izin</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="hadir_sore" id="sore_tidak" value="Tidak Hadir" {{ old('hadir_sore', $absenPertama->hadir_sore) == 'Tidak Hadir' ? 'checked' : '' }}>
                            ...
                            <div class="row mb-3" id="container_catatan" style="display: none;">
                                ...
                                <div class="col-sm-9">
                                    <textarea name="catatan" id="catatan" class="form-control mb-3" rows="3" placeholder="Masukkan alasan / catatan (Izin / Tidak Hadir)...">{{ old('catatan', $absenPertama->catatan ?? '') }}</textarea>
                                    ...
                                    <div id="preview_container" class="mt-3" style="{{ !empty($absenPertama->bukti_izin) ? '' : 'display: none;' }}">
                                        <small class="text-muted fw-semibold d-block mb-1">Preview Bukti:</small>
                                        <a id="preview_link" href="{{ !empty($absenPertama->bukti_izin) ? asset('storage/' . $absenPertama->bukti_izin) : '#' }}" target="_blank">
                                            <img id="image_preview" src="{{ !empty($absenPertama->bukti_izin) ? asset('storage/' . $absenPertama->bukti_izin) : '' }}" class="img-thumbnail rounded shadow-sm" style="max-height: 220px; object-fit: contain;">
                                        </a>
                                    </div>
                                </div>
                            </div>
            ...
            $('#bukti_izin').on('change', function(e) {
                ...
                if (file) {
                    ...
                    @if(!empty($absenPertama->bukti_izin))
                        $('#image_preview').attr('src', "{{ asset('storage/' . $absenPertama->bukti_izin) }}");
                        $('#preview_link').attr('href', "{{ asset('storage/' . $absenPertama->bukti_izin) }}");
