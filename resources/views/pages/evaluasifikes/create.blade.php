...
<style>
    ...
    .eval-header-card { background-color: #823ca2 !important; color: #ffffff; border-radius: 10px 10px 0 0; padding: 16px 24px; font-weight: 700; font-size: 1.1rem; letter-spacing: 0.5px; border-top: 4px solid #ff9c00; }
    ...
    .eval-radio-col input[type="radio"] { width: 20px; height: 20px; cursor: pointer; accent-color: #823ca2; margin: 0; }
    ...
    .eval-input-line:focus { outline: none; border-bottom-color: #ff9c00; box-shadow: none; }
    ...
    .eval-top-bar { height: 10px; background: linear-gradient(90deg, #823ca2 0%, #ff9c00 100%); }
    ...
    @media (max-width: 576px) {
        ...
        .eval-required-note { font-size: 0.78rem; }
        ...
        .eval-header-body p { font-size: 0.85rem !important; }
...
<div class="pagetitle">
    <h1>Isi Evaluasi FIKes (Fakultas Ilmu Kesehatan)</h1>
    <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item"><a href="{{ route('evaluasifikes.index') }}">Evaluasi</a></li>
            <li class="breadcrumb-item active">Isi Form</li>
        </ol>
    </nav>
</div>
...
<div class="container-fluid py-3">
    <div class="eval-container">
        <form action="{{ route('evaluasifikes.store') }}" method="POST" id="evalForm">
            @csrf
                            ...
                            <div class="eval-radio-options">
                                @for($i = 1; $i <= 4; $i++)
                                    <div class="eval-radio-col">
                                        ...
                                        <input type="radio" name="{{ $key }}" id="{{ $key }}_{{ $i }}" value="{{ $i }}" {{ old($key) == $i ? 'checked' : '' }} required>
                                    </div>
                                @endfor
                            </div>
            ...
            <!-- Header Section 3: Saran dan Masukan -->
                ...
                <div class="eval-card">
                    ...
                    <div class="mt-3">
                        <textarea name="{{ $fieldKey }}"
                                  ...
                                  rows="2"
                                  placeholder="Jawaban Anda">{{ old($fieldKey) }}</textarea>
                        @error($fieldKey)<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                </div>
            @endforeach

            <!-- Header Section 4: Fasilitas dan Penyelenggara -->
            ...
            <!-- Header Section 5: Sarana dan Prasarana -->
            ...
            <div class="eval-card" style="border-radius: 0 0 10px 10px; margin-bottom: 20px;">
                <div class="mt-1">
                    <textarea name="saran_panitia"
                              ...
                              rows="3"
                              placeholder="Jawaban Anda">{{ old('saran_panitia') }}</textarea>
            ...
            <div class="eval-action-bar">
                <div>
                    @if(Auth::user()->role == 'mahasiswa')
                        <a href="{{ route('dashboard') }}" class="btn btn-outline-secondary px-4 me-2">Kembali</a>
                    @else
                        <a href="{{ route('evaluasifikes.index') }}" class="btn btn-outline-secondary px-4 me-2">Kembali</a>
                    @endif
                    <button type="submit" class="btn text-white px-4" style="background-color: #823ca2; border-color: #823ca2;">
                        Simpan Evaluasi
                    </button>
                </div>
                <div>
                    <button type="reset" class="btn btn-link text-decoration-none text-muted p-0">Kosongkan formulir</button>
                </div>
            </div>
        </form>
    </div>
